...
class ObligationExitContractTests : ContractTest(CORDAPPS, CONTRACTS) {

    @Test
    fun `When exiting an obligation, the transaction must include the Exit command`() {
        services.ledger {
            transaction {
                input(ObligationContract.ID, FULLY_SETTLED_OBLIGATION_STATE)
                fails()
    ...
    fun `When exiting an obligation, zero output states must be created`() {
        services.ledger {
            transaction {
                input(ObligationContract.ID, FULLY_SETTLED_OBLIGATION_STATE)
                output(ObligationContract.ID, FULLY_SETTLED_OBLIGATION_STATE)
                command(keysOf(IDENTITY_A, IDENTITY_B), ObligationContract.Exit())
    ...
    fun `When exiting an obligation, the input state must be fully settled`() {
        services.ledger {
            transaction {
                input(ObligationContract.ID, PARTIALLY_SETTLED_OBLIGATION_STATE)
                command(keysOf(IDENTITY_A, IDENTITY_B), ObligationContract.Exit())
